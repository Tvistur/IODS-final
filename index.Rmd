...
The aim of this study is to find out how intensifying adverbs were used in historical spoken language. This will be done in three stages. First, following Macaulay's study, I will investigate whether a disctinction between the use of intensifying adverbs and social class can be identified in 18th-century courtroom discourse. After this, I will broaden the scope and fit a logistic regression model in order to predict the use of derived adverbs as intensifiers. Finally, I will furher evaluate the logiscti model and visualize the data by means of multiple correspondence analysis.
...
B. Social class:  
Speakers have been divided into two social classes, higher and lower, as explained in section 2.2. The fact that the majority of speakers come from the higher class is somewhat surprising. Judges and lawyers naturally belong to the this group but that does not explain the difference between the classes. One explanation could be the types of offences included in this data. Offences dealt with at the Old Bailey varied from pocketpicking to royal offences. Although the former were more common and were more likely to involve members of the lower class, it could be that this data does not represent the typical distribution of offences. The offence category is not included in the OBC metadata so this matter will remain unknown as far as this study is concerned.
...
Finally, in order to evaluate the model's performance, I have calculated the average prediction error and used it to performe a 10-fold cross-validation. The numerical result represents the average number of wrong predictions so the lower it is, the better. 
...
Let us take a brief look of MCA. I have chosen to include this in order to confirm my findings as well as to provide a more visual represenation of the data as a whole. MCA is a technique which reduces dimensions and by doing so it enables us to identify connections withing the data.
...
The summary above showed that the most influential variables in dimension 1 were role and class. Two roles in particular are highlighted, witness and victim, which were the most frequent ones in this data set. They are leaning towards the lower class. Lower class is associated with female speakers, and also the futhest away from derived intensifiers. Higher class is not as closely related to any particular role and is somewhere in the middle ground between the two types of intensifiers. Male speakers are very close to higher class.
  
Time (year) is key in both dimensions. Time is not, however, cronologically distributed along the dimensions but it would appear that some difference between 1758 and 1795 has been captured. What is most of interest is the year 1815 which is set apart by the second dimension, along with extreme instances recorded in the 1780s.  
  
# Discussion

Although the logistic model performed surprisingly well and the multiple correspondence analysis confirmed the key findings, both have to be taken with a pinch of salt - they cannot be claimed to be exhaustive representations of all linguistic behaviour in this data set. As was mentioned in the beginning, the data is based on the *Proceedings* which were not complete accounts and they were subject to scribal and other kinds of interference. Also, if something has been left out, it does not mean that it did not happen, but for the researcher it is inevitably lost and must be taken into account when analysing this kind of data. Neither should we forget the severe skewness of the data which we saw especially in the case of gender and social class - for example, there was very little data representing language spoken by higher class women. So, what the analysis has captured is the linguistic behaviour of *higher class males* given the records available to us from this one source.
  
One of the predictor variables in the model was the speaker's role in the courtroom. It could be argued that it is more a arbitrary variable since men and women from different social classes would have, at least to some extent, occupied different roles in the courtroom depending on the trial. So, in this way it is a more circumstancial than a descriptive variable. However, I chose to include it in the model because the official nature of the context could have brough about code switching, e.g. lower class speakers making use of the speech habits of the more prestigious higher class speakers in an attempt to add weight to what they were saying. 
